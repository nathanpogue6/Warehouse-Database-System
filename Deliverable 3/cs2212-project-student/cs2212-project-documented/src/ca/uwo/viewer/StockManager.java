...
public class StockManager extends Viewer implements Runnable {
	...
	private StockManager() {
		...
		//restockStrategy = RestockStrategyFactory.create("strategy1");
		t.start();
	}
	...
	public void inform(Item item) {
		int var = restockStrategy.calculateQuantity(item.getName(), item.getAvailableQuantity(), item.getPrice());
		restockDetails.put(item.getName(), var);
	}
